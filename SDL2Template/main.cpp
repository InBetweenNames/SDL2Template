              ...
              Eigen::Matrix4f const& view, SDL_Surface* display)
{
    ...
    Eigen::Vector4f const p1ViewBot = view * Eigen::Vector4f{p1(0), offset, p1(1), 1};
    Eigen::Vector4f const p1ViewTop = view * Eigen::Vector4f{p1(0), offset + height, p1(1), 1};
    Eigen::Vector4f const p2ViewBot = view * Eigen::Vector4f{p2(0), offset, p2(1), 1};
    Eigen::Vector4f const p2ViewTop = view * Eigen::Vector4f{p2(0), offset + height, p2(1), 1};

    // Perspective division
    Eigen::Vector4f const p1ProjBot = c * p1ViewBot / p1ViewBot(2);
    Eigen::Vector4f const p1ProjTop = c * p1ViewTop / p1ViewTop(2);
    Eigen::Vector4f const p2ProjBot = c * p2ViewBot / p2ViewBot(2);
    Eigen::Vector4f const p2ProjTop = c * p2ViewTop / p2ViewTop(2);
    ...
    float const topXDiff = p2ProjTop(0) - p1ProjTop(0);
    float const botXDiff = p2ProjBot(0) - p1ProjBot(0);

    float const topFrac = (p2ProjTop(1) - p1ProjTop(1)) / topXDiff;
    float const botFrac = (p2ProjBot(1) - p1ProjBot(1)) / botXDiff;
    ...
    float currTopY =
        display->h / 2
        - (p1ProjTop(1) + float(startColumn - startColumnUnclamped) * (p2ProjTop(1) - p1ProjTop(1)) / topXDiff);

    float currBotY =
        display->h / 2
        - (p1ProjBot(1) + float(startColumn - startColumnUnclamped) * (p2ProjBot(1) - p1ProjBot(1)) / botXDiff);

    float currX = float(startColumn - startColumnUnclamped) / topXDiff;

    for (ptrdiff_t i = startColumn; i <= endColumn; i++)
    {
        DrawColumn(display, selectedTex, i, static_cast<ptrdiff_t>(currTopY), static_cast<ptrdiff_t>(currBotY), currX,
                   0, 1);
...
void Draw(GameResources* res, uint32_t const frameTime[2])
{
    ...
    DrawWall(res->wallFrontTranspose, res->wallBackTranspose, Eigen::Vector2f{-300, -300}, Eigen::Vector2f{300, -300},
             0, 150, res->dir.c, res->view, display);
